...
/* ---- Staggered card/list entrance, re-triggered every render ---- */
function staggerIn(){ /* Intentionally stable: edits do not replay list animations. */ }
...
function load(key, fallback){
  try {
    ...
    // Keep the original bytes intact so a damaged section can be recovered.
...
function buildICS(){
  ...
  schedule.forEach(ev=>{
    ...
    if(ev.freq === 'WEEKLY' && ev.byday && ev.byday.length){
      let rrule = 'RRULE:FREQ=WEEKLY;BYDAY=' + ev.byday.join(',');
...
function exportICS(){
  const totalEvents = schedule.length + classes.filter(c=>c.source!=='ics').length + deadlines.filter(d=>d.date).length;
  if(totalEvents === 0){ toast('Nothing to export yet — add some classes or deadlines first.', true); return; }
  const ics = buildICS();
...
function renderTodos(){
  ...
  staggerIn(todoListEl, '.todo-item');
  renderDailyPlanner();
}
...
let incomeLog = load('wit_income_log', {}); // { "<incomeName>|<weekday>": "YYYY-Www" }

function isoWeekKey(date){
  // Simple Mon-start week key like "2026-W07" — good enough to dedupe
  // "have we already logged this source for this calendar week" checks.
...
function extractTimePhrase(text){
  ...
  if(m){
    ...
    const label = new Date(2000,0,1,h,min).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'});
    return { label, text: qcStripPhrase(text, m[0]) };
  }
  m = text.match(/\b(?:at\s+)?([01]?\d|2[0-3]):([0-5]\d)\b/);
  if(m){
    const label = new Date(2000,0,1,parseInt(m[1],10),parseInt(m[2],10)).toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'});
    return { label, text: qcStripPhrase(text, m[0]) };
  }
  return null;
}
...
function renderBudgetSummary(){
  ...
  staggerIn(document.getElementById('budgetSummary'), '.budget-pill');
}
...
function renderTransactions(){
  ...
  el.innerHTML = sorted.slice(0,25).map(t=>{
    const sign = t.type === 'income' ? '+' : '−';
...
function eventOccursOnDate(ev, date){
  ...
  if(rec.freq === 'monthly'){
    ...
    if(countLimit !== null && (monthDiff / interval) >= countLimit) return false;
    return true;
  }
  if(rec.freq === 'weekly'){
...
function gatherState(){
  return {
    ...
    transactions, recurringBills, simBudget,
...
function validateBundle(bundle){
  ...
  const schemas = {
    ...
    coop: r => record(r,{company:text,role:text,link:text,notes:text,status:v=>COOP_STATUSES.includes(v)}) && text(r.company),
...
function applyCloudState(bundle){
  ...
  const keys = {links:'links',schedule:'schedule',classes:'classes',deadlines:'deadlines',events:'events',coop:'coop',todos:'todos',transactions:'transactions',recurringBills:'recurring',simBudget:'simBudget',grades:'grades',recurringIncome:'recurring_income',incomeLog:'income_log',countdown:'countdown',habits:'habits',semester:'semester',focusSessions:'focus_sessions',gpaHistory:'gpa_history'};
  ...
  ({links,schedule,classes,deadlines,events,coop,todos,transactions,recurringBills,simBudget,grades,recurringIncome,incomeLog,countdown,habits,semester,focusSessions,gpaHistory} = state);
...
async function handleUnlockSubmit(){
  ...
  try{
    ...
    if(payload){
      try{
        bundle = await decryptForSync(payload, key);
      } catch(e){
...
function openInsightsModal(){
  document.getElementById('insightsOverlay').classList.add('open');
  renderInsights();
}
function closeInsightsModal(){
  document.getElementById('insightsOverlay').classList.remove('open');
}
...
function openHabitsModal(){
  document.getElementById('habitsOverlay').classList.add('open');
  renderHabits();
}
function closeHabitsModal(){
  document.getElementById('habitsOverlay').classList.remove('open');
}
...
function enhanceControls(root){
  root.querySelectorAll('input:not([type="hidden"]),select,textarea').forEach(el=>{
    ...
    const label = el.getAttribute('placeholder') || el.id.replace(/([a-z])([A-Z])/g,'$1 $2') || 'Value';
  ...
  root.querySelectorAll('.icon-btn,.del-btn').forEach(el=>{
    ...
    const context = el.closest('.card,.todo-item,.class-card,.kanban-card,tr,.habit-row');
    const title = context?.querySelector('h2,[contenteditable]')?.textContent.trim() || 'item';
...
for(const overlay of overlays){
  ...
  else panel.setAttribute('aria-label',overlay.id==='paletteOverlay' ? 'Commands' : 'Dashboard dialog');
  new MutationObserver(()=>{
    const open = overlay.classList.contains('open');
    if(open && !dialogOpeners.has(overlay)){
      dialogOpeners.set(overlay,document.activeElement);
      (panel.querySelector('input,select,textarea,button,[tabindex="0"]') || panel).focus();
    } else if(!open && dialogOpeners.has(overlay)){
      const opener = dialogOpeners.get(overlay); dialogOpeners.delete(overlay);
